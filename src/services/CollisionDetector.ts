...
export default class CollisionDetector {
     ...
     */
    checkHorizontalCollisions(entity: Entity): void {
     ...
     * @param {Entity} entity - The entity to check collisions for.
     */
    checkVerticalCollisions(entity: Entity): void {
        const tileSize = this.map.tileSize;
        const tileLeft = Math.floor(entity.position.x / tileSize);
        const tileRight = Math.floor(
     ...
     */
    isSolidTileInColumn(x: number, yStart: number, yEnd: number): boolean {
     ...
     * @returns {boolean} True if a solid tile is found, false otherwise.
     */
    isSolidTileInRow(y: number, xStart: number, xEnd: number): boolean {
     ...
     */
    checkBlockCollisionFromBelow(entity: Entity, tileY: number, xStart: number, xEnd: number): boolean {
        for (let x = xStart; x <= xEnd; x++) {
            ...
            if (block && !block.isHit) {
                //Checks if the entity's top is close to the block's bottom
