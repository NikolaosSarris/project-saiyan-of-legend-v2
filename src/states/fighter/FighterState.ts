...
import { PlayerControls, PlayerControlMap } from "../../enums/PlayerControls";
...
export default class FighterState extends State {
     ...
     */
    getCollidingTiles(left: number, top: number, right: number, bottom: number): { x: number; y: number }[] {
        const collidingTiles: { x: number; y: number }[] = [];
